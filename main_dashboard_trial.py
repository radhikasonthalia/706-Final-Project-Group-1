...
# -------------------------------------------------------------------------
    ...
    # ---- EDUCATION FILTERING ----
    df_education = df[
        (df['setting'].isin(income_countries)) &
    ...
    valid_countries = sex_counts[
        (sex_counts.get("Male", 0) > 0) & (sex_counts.get("Female", 0) > 0)
    ].index.tolist()
    ...
    if selected_country_name:
        ...
        df_country = df_country[df_country["dimension"] == "Economic status (wealth quintile)"]
        ...
        with st.expander("ℹ️ More about this data"):
            ...
            **Data filtering details:**
            ...
            - Some countries may have **small sample sizes** for certain subgroups.
            """)

    # Footer
    st.markdown("---")
    st.caption(f"Data Source: Health determinants (DHS Program and UNICEF Data Warehouse)")
